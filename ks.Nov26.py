...
#-----------------------------------------------
   ...
   #----------------------------------------------------------------------------
        ...
        self.plvTree, _=  self.create_procListViewFrameGUI(procListTableFrame)  # Create the GUI for Table/List of Process 
        #           ^ ^^
        ...
        timeGraphTableFrame = tk.Frame(timeGraphFrame, relief='raised', borderwidth=20)         # Border (pop out) sunken to raised, 20
        self.create_timeGraphContentsGUI(timeGraphTableFrame)           # Create the GUI for Time Graph
        timeGraphTableFrame.grid(row=2,column=2,sticky='nsew')

        print('1')
        plvANDtgFrame.after(self.updateIntervalMS, self.periodicUpdateProcIDList)
...
#       # The window contains some entry fields
   ...
   #----------------------------------------------------------------
        ...
        PIDLabel.grid(row=4,column=0, sticky='e')  # right justify (e)
        self.PID_GuiScratchpad  = tk.Entry(plvFrame)
        self.PID_GuiScratchpad.grid(row=4,column=1)
        ...
        # ---- Initialize cols 0..3 to somewhat smaller size


        plvTree.columnconfigure(0, minsize=20, weight=1)
        plvTree.columnconfigure(1, minsize=20, weight=1)
        plvTree.columnconfigure(2, minsize=20, weight=1)
        plvTree.columnconfigure(3, minsize=20, weight=1)
        plvTree.columnconfigure(4, minsize=20, weight=6)

        plvTree.column(0, width=50)
        plvTree.column(1, width=50)
        plvTree.column(2, width=50)
        plvTree.column(3, width=50)
        plvTree.column(4, width=400)
        ...
        def eventClick(event):
           ...
           self.PID_GuiScratchpad.delete(0, END)
           self.PID_GuiScratchpad.insert(END, plvTree.item(val)['text'])
        ...
        def myEventDoubleClick(event):
            ...
            self.PID_GuiScratchpad.delete(0, END)
            self.PID_GuiScratchpad.insert(END, val)
        ...
        plvFrame.after(self.updateIntervalMS, self.periodicUpdateProcIDList)  # ----> periodic update
   ...
   #-----------------------------------------------------------------------------
        ...
        if(self.boolTakeInData): #for some reason this ins't getting the memo when pause is pressed.
            ...
            # pidDict = {}
            # print('---Contents of stdout follows----')   # Debug print:  data from stdout
            i=-1
            self.pidList=[]
   ...
   #-----------------------------------------------------------------------------
        ...
        self.cpuFrame = tk.Frame(parent)
        self.memFrame = tk.Frame(parent)
        self.idkFrame = tk.Frame(parent)

        self.cpuFrame.grid( row=2, column=2,sticky='nsew')
        self.memFrame.grid( row=4, column=2,sticky='nsew')
        self.idkFrame.grid( row=6, column=2,sticky='nsew')
        ...
        # tk.Button(parent, text='tgBut1').grid(row=2, column=2,sticky='nsew',padx=5)
        tk.Button(parent, text='tgBut2').grid(row=4, column=2,sticky='nsew',padx=5)
        tk.Button(parent, text='tgBut3').grid(row=6, column=2,sticky='nsew',padx=5)

        tk.Label(parent, text='CPU',  relief='sunken',borderwidth=2).grid(row=2, column=2, sticky='nw') # Top of Frame:    "CPU"
        tk.Label(parent, text='MEM',  relief='sunken',borderwidth=2).grid(row=4, column=2, sticky='nw') # Bottom of Frame: "MEM"
        tk.Label(parent, text='OTHER',relief='sunken',borderwidth=2).grid(row=6, column=2, sticky='nw') # Bottom of Frame: "OTHER"


        ####matplotlib###
        self.matplotFrame = tk.Frame(self.cpuFrame, relief='sunken',bg='blue')

        self.matplotFrame.pack(fill='both', padx=2, pady=10)
        ...
        self.ax.set_title('axes 1 title')                     # Title of Graph.  Centered.  Above Graph.
        self.ax.set_ylabel('% Consumption-Ylabel')            # Title of Left   (Y) 
        self.ax.set_xlabel('Time(s)')                         # Title of Bottom (X) 
        ...
        self.ax.plot(t, s)                                    # Plot the X (t)  and Y (s) values

        # a tk.DrawingArea
        self.canvas = FigureCanvasTkAgg(fig, master=self.matplotFrame)
        self.canvas.show()
        self.canvas.get_tk_widget().pack(side='top', fill='both', expand=1)

        toolbar = NavigationToolbar2TkAgg(self.canvas, self.matplotFrame)
        toolbar.update()
        ...
        self.matplotFrame.after(self.updateIntervalMS, self.periodicUpdateOfGraph)    # ----> periodic update
